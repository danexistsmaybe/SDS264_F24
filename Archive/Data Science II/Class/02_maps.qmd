...
(states_gon <- states_polygon |>
  mutate(state = str_to_lower(str_replace(region, " ", "")))
) |> print(n = 500)

(states_sf <- states_sf |>
  mutate(state = str_to_lower(str_replace(name, " ", "")))
) |> print(n = 500)
```

```{r}
# Now we can merge data sets together for the static and the interactive plots
(merged_poly <- states_gon |>
  left_join(poly, by = "state")
)
...
pal <- colorBin("YlOrRd", domain = merged_sf$abortion_rank12, bins = bins)
...
merged_sf <- merged_sf |>
  mutate(labels = str_c(name, ": ", abortion_rank12, " people / sq mile"))

# If want more HTML formatting, use these lines instead of those above:
#states <- states |>
#  mutate(labels = glue("<strong>{name}</strong><br/>{density} people / #mi<sup>2</sup>"))

labels <- lapply(merged_sf$labels, HTML)

leaflet(merged_sf) |>
  setView(-96, 37.8, 4) |>
  addTiles() |>
  addPolygons(
    fillColor = ~pal(abortion_rank12),
    weight = 2,
    opacity = 1,
    color = "white",
    dashArray = "3",
...
pal <- colorFactor("YlOrRd", domain = merged_sf$abort_rank3)
...
merged_sf <- merged_sf |>
  mutate(labels = str_c(name, ": ", abort_rank3, " people / sq mile"))
